...
export default function App() {
  ...
  async function loadDashboard() {
    ...
    /*
     * تحميل جميع سجلات الحضور
     * على دفعات حتى لا نتوقف عند أول 1000 سجل
     */
    let allAttendance = [];
    let from = 0;
    const pageSize = 1000;
    ...
    /*
     * حساب حضور اليوم مباشرة من Supabase
     * الدالة ترجع العدد الصحيح للحضور
     */
    const {
      data: todayPresent,
      error: todayPresentError,
    } = await supabase.rpc(
      "get_today_present_count"
    );

    if (todayPresentError) {
      console.error(
        "خطأ في حساب حضور اليوم:",
        todayPresentError
      );

      /*
       * في حالة فشل الدالة، نحسب محليًا
       * كحل احتياطي
       */
      const today =
        new Date()
      ...
      setPresentToday(fallbackCount);
    } else {
      setPresentToday(
        Number(todayPresent || 0)
      );
    }
  }
...
export default function App() {
  ...
  async function loadDashboard() {
    ...
    /*
     * تحميل جميع سجلات الحضور على دفعات
     * بدل الاعتماد على أول 1000 سجل فقط
     */
    let allAttendance = [];
    let from = 0;
    const pageSize = 1000;
